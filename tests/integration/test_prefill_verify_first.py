...
"""Verify-first regression for regular coord prefill."""
...
def _snapshot_tree(target_path: Path) -> dict[str, tuple[int, int, bytes]]:
    coord_chunk_root = target_path / _GROUP / _COORD_NAME / "c"
    if not coord_chunk_root.exists():
        return {}
    return {
        p.relative_to(coord_chunk_root).as_posix(): (
            p.stat().st_mtime_ns,
            p.stat().st_size,
            p.read_bytes(),
        )
        for p in coord_chunk_root.rglob("*")
        if p.is_file()
    }


def test_prefill_fresh_fill_materializes_full_grid(tmp_path: Path) -> None:
    target_path = tmp_path / "cube.zarr"
    plugin_name = "prefill_verify_first_full"
...
def test_prefill_matching_resume_is_no_op(tmp_path: Path) -> None:
    target_path = tmp_path / "cube.zarr"
    plugin_name = "prefill_verify_first_noop"
    _register_regular_plugin(plugin_name)
    runner = CliRunner()

    first = runner.invoke(cli, _preallocate_args(plugin_name, plugin_name, target_path))
    assert first.exit_code == 0, first.output
    before = _snapshot_tree(target_path)

    second = runner.invoke(cli, _preallocate_args(plugin_name, plugin_name, target_path))

    assert second.exit_code == 0, second.output
    assert "no-op (matches nominal grid)" in second.output
    assert _snapshot_tree(target_path) == before


def test_prefill_divergent_resume_raises(tmp_path: Path) -> None:
    target_path = tmp_path / "cube.zarr"
    plugin_name = "prefill_verify_first_divergent"
    _register_regular_plugin(plugin_name)
    runner = CliRunner()
    ...
    assert first.exit_code == 0, first.output
    coord = cast(Any, _root(target_path, mode="r+")[f"{_GROUP}/{_COORD_NAME}"])
    coord[5] = _EXPECTED_VALUES[5] + np.timedelta64(1, "s")
    before = _snapshot_tree(target_path)
    ...
    assert "slot 5" in combined, combined
    assert "stored" in combined, combined
    assert "incoming" in combined, combined
    assert _snapshot_tree(target_path) == before
