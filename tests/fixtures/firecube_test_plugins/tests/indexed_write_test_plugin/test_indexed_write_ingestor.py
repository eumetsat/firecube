...
Every ingestor's ``index_spec`` / ``build_write_intents`` hook is exercised
...
def test_single_ingestor_returns_one_indexed_write_per_item() -> None:
    ...
    results = ingestor.build_write_intents(_batch(items), ctx)
...
def test_fan_out_ingestor_returns_two_indexed_writes_per_item() -> None:
    ...
    for idx, item in enumerate(items):
        seq = ingestor.build_write_intents(_batch([item]), ctx)
        assert len(seq) == 2
        first, second = seq
...
def test_drop_ingestor_skips_item_two() -> None:
...
def test_error_ingestor_compilation_raises_indexed_write_compilation_error() -> None:
    ...
    (iw,) = ingestor.build_write_intents(_batch([0]), ctx)
