...
"""Test fixture plugins for the mixed ``build_write_intents`` compile path.

Five ingestor variants exercise the merged ``DirectZarrIngestor.build_write_intents``
contract end-to-end without touching disk (items are pure integers 0..N-1):

* ``IndexedWriteSingleIngestor`` -- happy path, one ``IndexedWrite.region`` per
  item, five slots on a 300s ``RegularTimeAxis``.
* ``IndexedWriteFanOutIngestor`` -- fan-out: each item appends two
  ``IndexedWrite.region`` writes to different row slices at the same slot.
* ``IndexedWriteWithStaticsIngestor`` -- mixes ``IndexedWrite.region`` elements
  with a ``WriteIntent.static`` for a ``lat`` grid in one returned list.
* ``IndexedWriteErrorIngestor`` -- emits an ``IndexedWrite`` whose
...
* ``IndexedWriteDropIngestor`` -- skips item 2 by not appending a write for it.
...
class IndexedWriteWithStaticsIngestor(DirectZarrIngestor):
    ...
    ``build_write_intents`` returns one region ``IndexedWrite`` per item plus a
    ``WriteIntent.static`` for the static ``lat`` array in the same list,
    mirroring the mixed-list contract documented on ``DirectZarrIngestor``.
    """
...
class IndexedWriteErrorIngestor(DirectZarrIngestor):
    ...
    emitted ``IndexedWrite`` uses a bogus ``"NOT_IN_INDEX"`` string that
...
class IndexedWriteDropIngestor(DirectZarrIngestor):
    """Drop-item fixture: no write is appended for item 2."""
    ...
    def build_write_intents(
        ...
        for item in batch.items:
            ...
            if item == self._DROP_INDEX:
                continue
            out.append(_region_write_for(item))
        return out


def _region_write_for(item: int) -> IndexedWrite:
