...
class SlotGroupCapable(DirectZarrIngestor):
    ...
    def global_expected_time_count(self, ctx: PluginContext) -> dict[str, int] | None:
        _ = ctx
        return {spec.group: 100 for spec in self._schema}
    ...
    def slot_index_model(self, ctx: PluginContext) -> SlotIndexModel:
        ...
        return SlotIndexModel(
            ...
            groups={spec.group: SlotAxis(cadence_s=1, mode="exact") for spec in self._schema},
        )
...
def test_unknown_slot_group_fails_before_writes() -> None:
    ...
    with pytest.raises(ConfigurationError) as exc_info:
        validate_parallel_capability(ingestor, 0, 10, ctx=None, slot_group="missing_grp")  # type: ignore[arg-type]
...
def test_known_slot_group_passes_capability_gate() -> None:
    ...
    result = validate_parallel_capability(ingestor, 0, 10, ctx=None, slot_group="group_a")  # type: ignore[arg-type]

    assert result == {"group_a": 100, "group_b": 100}
...
def test_slot_group_none_validates_all_groups_chunks() -> None:
    ...
    with pytest.raises(ConfigurationError, match="group_b"):
        validate_parallel_capability(ingestor, 0, 10, ctx=None, slot_group=None)  # type: ignore[arg-type]
...
def test_slot_group_set_validates_misalignment_for_that_group() -> None:
    ...
    with pytest.raises(ConfigurationError, match="group_a"):
        validate_parallel_capability(ingestor, 0, 10, ctx=None, slot_group="group_a")  # type: ignore[arg-type]
...
def test_capability_gate_skipped_when_no_slot_flags() -> None:
    class ExplodingIngestor(SlotGroupCapable):
        ...
        def global_expected_time_count(self, ctx: PluginContext) -> dict[str, int] | None:
            raise AssertionError("global schema should not be queried")
...
def test_unknown_group_error_message_includes_available_groups() -> None:
    ...
    with pytest.raises(ConfigurationError) as exc_info:
        validate_parallel_capability(ingestor, 0, 10, ctx=None, slot_group="bbb")  # type: ignore[arg-type]
